...
| 購物車 | 移除商品 | ✅ 完成 |
| 購物車 | 清空購物車 | ✅ 完成 |
| 配送 | 宅配／超商、滿額免基本運費與附加費計算 | ✅ 完成 |
| 訂單 | 建立訂單（含配送費、庫存扣除與購物車清空） | ✅ 完成 |
| 訂單 | 查看我的訂單列表 | ✅ 完成 |
| 訂單 | 查看訂單詳情 | ✅ 完成 |
...
**POST /api/orders**
- 需要登入
- 從購物車建立訂單、計算配送費、扣減庫存、清空購物車
- 訂單狀態初始為 `pending`
- `shippingMethod` 必填，可選 `home_delivery` 或 `cvs`
- 商品小計滿 NT$ 1,500 免基本運費；偏遠地區／當日急件附加費仍會疊加
