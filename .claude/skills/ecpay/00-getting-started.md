...
> | **非 PHP 必讀** | [guides/13](./13-checkmacvalue.md) + [guides/14](./14-aes-encryption.md) + [guides/20](./20-http-protocol-reference.md) |
> | **遇到問題** | [guides/15](./15-troubleshooting.md)（按症狀）或 [guides/21](./21-error-codes-reference.md)（按錯誤碼） |
...
>
> ⚠️ **此路徑不測試 ReturnURL callback**：若你的訂單狀態、庫存、發票等業務邏輯依賴 ReturnURL callback 通知，請改用 ngrok 方案（見下方「本地開發環境」）進行完整測試。完整本地開發環境設定（ngrok / Cloudflare Tunnel / RequestBin）見 [guides/25 本地開發環境設定](./25-local-development.md)。
>
...
>
> 詳細設定見 [guides/25 本地開發環境](./25-local-development.md)。
...
| 非 PHP 語言完整範例 (Go/Java/C#/TS/Kotlin/Ruby) | [guides/24-multi-language-integration.md](./24-multi-language-integration.md)（⚠️ 約 800 行，使用 AI Section Index 跳轉） |
| 完整決策樹 | SKILL.md 步驟 2 |
...
| Node.js / Python / TypeScript | 本指南 → `lang-standards/{語言}.md` → guides/13 → guides/20 → guides/01（已有 Quick Start） |
| Go / Java / C# / Kotlin / Ruby | 本指南 → `lang-standards/{語言}.md` → guides/13 → guides/14 → guides/24（完整 E2E） |
| Swift / Rust | 本指南 → `lang-standards/{語言}.md` → guides/13 → guides/14 → guides/24（CLI 範例 + Mobile App） |
| C / C++ | 本指南 → `lang-standards/{語言}.md` → guides/13 → guides/14 → guides/20（HTTP 協議 → 自行整合） |
...
每個 `lang-standards/*.md` 涵蓋：命名慣例、HTTP client 設定、timing-safe 比對函式、callback handler 模板、AES/CMV 陷阱提醒。
