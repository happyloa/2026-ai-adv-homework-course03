...
> | 全方位物流 v2 | `Timestamp` + `Revision` | 詳見 [guides/07](./07-logistics-allinone.md) |
...
| AES-JSON + CMV（直播收款） | JSON POST（四層結構，含 ECTicket 式 CheckMacValue SHA256） | `1\|OK`（⚠️ 驗證方式同電子票證，但回應為純文字） |
